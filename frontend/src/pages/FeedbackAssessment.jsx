...
function FeedbackAssessment() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const feedbackData = {
        ratingQuestions: Object.entries(answers).map(([id, response]) => {
        ...
        openEndedQuestions: Object.entries(openEndedAnswers).map(([id, response]) => {
          const question = questions.openEndedQuestions.find(q => q.id === id);
          return {
            id,
            text: question.text,
            response
        ...
        overallProgress: ((currentQuestionIndex + 1) / allQuestions.length) * 100,
        ...
        <div className="mx-auto max-w-3xl">
          ...
          >
            ...
            <div className="mt-2 flex justify-center text-center text-sm text-gray-600 sm:text-base">
              ...
              <span className="ml-1 mr-1 font-medium text-[#0029ff]">
            ...
            {!showOpenEnded ? (
              ...
              <div className="space-y-6">
                {questions.openEndedQuestions.map((question) => (
                  <div key={question.id} className="rounded-xl border border-gray-100 p-6 shadow-md">
                    <h4 className="mb-4 text-lg font-medium text-gray-800">{question.text}</h4>
                    <textarea
                      className="w-full text-gray-600 rounded-lg border border-gray-200 p-3 focus:border-[#0029ff] focus:outline-none focus:ring-1 focus:ring-[#0029ff]"
                      rows={4}
