  ...
  const handleFeedbackSelection = async (decision) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/chat-box/save-feedback`,
        {
          method: "POST",
          headers: {
    ...
    >
      ...
      >
        ...
        <p className="text-sm font-medium">
          How is the {chatType === "coaching" ? "Coaching" : "Roleplay"} going?
          Do you feel ready?
        </p>
      </motion.div>
      ...
      <div className="space-y-3">
        ...
        >
          <FiRepeat />
          <p className="text-sm">
            No, continue with the{" "}
            {chatType === "coaching" ? "Coaching" : "Roleplay"}
          </p>
        </motion.button>
        {/* 
        <motion.button
